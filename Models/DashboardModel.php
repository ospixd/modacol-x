...
class DashboardModel extends Mysql
{
    ...
    public function cantClientes(){
        $sql = "SELECT COUNT(*) as total FROM persona WHERE rolid = 8 AND status != 0";
        $request = $this->select($sql);
        $total = $request['total'];
        return $total;
    ...
    public function lastOrders(){
        ...
        $sql = "SELECT p.idpedido, CONCAT(pr.nombres,' ',pr.apellidos) as nombre, p.monto, p.status, p.referenciacobro
                FROM pedido p
    ...
    public function selectVentasMes(int $anio, int $mes){
        ...
        for ($i=0; $i <  $dias ; $i++) {
            ...
            FROM pedido WHERE DATE(fecha) = '$fechaVenta' AND status = 'Aprobado' ".$where;
    ...
    public function selectVentasAnio(int $anio){
        ...
        for($i=1; $i <= 12; $i++){
            ...
            $sql = "SELECT $anio AS anio, $i AS mes, SUM(monto) AS venta FROM pedido
            WHERE MONTH(fecha) = $i AND YEAR(fecha) = $anio AND status = 'Aprobado'
