...
class PedidosModel extends Mysql
{
    ...
    public function selectDireccion(string $idtransaccion, $idpersona = NULL){
        $busqueda = "";
        if($idpersona != NULL){
            $busqueda = " AND p.personaid =".$idpersona;
        }
        $sql = "SELECT direccion_envio,
                        DATE_FORMAT(fecha, '%d/%m/%Y') as fecha 
                        FROM pedido 
                        WHERE idtransaccionpaypal = $idtransaccion ".$busqueda;
        $request_direccion = $this->select($sql);
        $request = array('datos' => $request_direccion);
        return $request;
    }


    public function selectTransWompi(string $idtransaccion, $idpersona = NULL){
        $busqueda = "";
        if($idpersona != NULL){
        ...
        if(!empty($requestData)){
            $objData = unserialize($requestData['datospaypal']);
            $referencia = $objData->data->reference;
            $objDatos = getDatosWompi($referencia);
        }
        return $objDatos;
    ...
    public function selectTransPaypal(string $idtransaccion, $idpersona = NULL){
        ...
        if(!empty($requestData)){
            ...
            $urlTransaccion = URLPAYPAL."/payments/captures/".$idtransaccion;
    ...
    public function reembolsoPaypal(string $idtransaccion, string $observacion){
        ...
        if(!empty($requestData)){
            $objData = json_decode($requestData['datospaypal']);
            $urlReembolso = URLPAYPAL."/v2/payments/captures/".$idtransaccion."/refund";
