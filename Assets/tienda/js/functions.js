    ...
    $(this).on('click', function(){
        ...
        request.onreadystatechange = function (){
            ...
            if(request.status == 200){
                ...
                Swal.fire(nameProduct, objData.msg, "success");
                }else{
    ...
    optmetodo.forEach(function(optmetodo) {
        optmetodo.addEventListener('click', function(){
            if(this.value == "Paypal"){
                ...
                document.querySelector("#divwompi").classList.add("notblock");
            }else if(this.value =="wompi"){
                document.querySelector("#divpaypal").classList.add("notblock");
                document.querySelector("#divtipopago").classList.add("notblock");
                document.querySelector("#divwompi").classList.remove("notblock");
            }else{
                document.querySelector("#divpaypal").classList.add("notblock");
...
if(document.querySelector("#btnComprar")){
    ...
    btnPago.addEventListener('click',function(){
        ...
        if(dir == "" || ciudad == "" || inttipopago == ""){
            Swal.fire("", "Complete los datos de envío", "error");
            return;
        }else{
                divLoading.style.display = "flex";
                let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
...
if(document.querySelector("#frmSub")){
    let frmSub = document.querySelector("#frmSub");
    frmSub.addEventListener('submit',function(e){
        e.preventDefault();
        let nombre = document.querySelector("#nombreSub").value;
        let email = document.querySelector("#emailSub").value;
        ...
        if(!fntEmailValidate(email)){
            Swal.fire("", "El email no es valido", "error");
            return false;
        }
                ...
                let formData = new FormData(frmSub);
                request.open("POST",ajaxUrl,true);
                request.send(formData);
                request.onreadystatechange = function(){
                    if(request.readyState != 4) return;
                    if(request.status == 200){
                        ...
                        if(objData.status){
                            ...
                            document.querySelector("#frmSub").reset();
                        }else{
                            Swal.fire("", objData.msg, "error");
                        }
                    }
